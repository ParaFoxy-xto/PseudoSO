class Memoria:
    def __init__(self, tamanho_total, blocos_tempo_real):
        ...
        self.memoria_ocupada = [-1] * tamanho_total  # Lista para rastrear a ocupação da memória

    def alocar_memoria(self, processo):
        if processo.eh_tempo_real:
            blocos_disponiveis = self.blocos_disponiveis_tempo_real
            inicio_base = 0  # Alocação começa do início da memória para processos de tempo real
        else:
            blocos_disponiveis = self.blocos_disponiveis_usuario
            inicio_base = self.blocos_tempo_real - 1  # Alocação começa após os blocos de tempo real

        if processo.blocos_de_memoria <= blocos_disponiveis:
            # Encontra uma região contígua para alocar o processo
            inicio = self.encontrar_inicio_regiao_contigua(inicio_base, processo.blocos_de_memoria, processo.eh_tempo_real)
            if inicio is not None:
                # Alocação bem-sucedida, atualiza o estado da memória e retorna o início da região alocada
                ...
                return inicio
        # Alocação falhou por falta de espaço
